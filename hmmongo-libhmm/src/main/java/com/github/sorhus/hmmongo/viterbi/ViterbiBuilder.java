...
public class ViterbiBuilder {
    ...
    public ViterbiBuilder(HMM hmm, int T) {
        this.impl = new ViterbiImpl(hmm, T);
    }

    private <I, O> Viterbi<I, O> build(Viterbi<I, O> viterbi) {
        return threadSafe ? new ThreadSafeViterbi<>(viterbi) : viterbi;
    }

    public ViterbiBuilder threadSafe() {
        threadSafe = true;
        return this;
    }

    public Viterbi<int[], int[]> withNoEncoderNoDecoder() {
        return build(impl);
    }

    public  <I, O> Viterbi<I, O> withEncoderDecoder(Encoder<I> encoder, Decoder<O> decoder) {
        return build(new ViterbiDecoder<>(new ViterbiEncoder<>(encoder), decoder));
    }
